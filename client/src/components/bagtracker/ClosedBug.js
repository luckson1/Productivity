...
function ClosedBugs({ children }) {
    ...
    const editBugHandler= (item)=> {
        ...
        const editedBugValues= {title: item?.bug.title, description: item?.bug.description, status:"Closed", _id:item?.bug._id, createdAt: item?.bug.createdAt,  bugId:item?.bug.bugId, }
        let editedBug=[]
