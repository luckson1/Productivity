...
const config = {
    headers: {
        ...
        try {
            ...
            const { data } = await axios.get(`${BaseURL}/teams/${payload?.id}`, config);
            return data;
...
// edit team
export const editteam = createAsyncThunk('teams/update', async (payload, { rejectWithValue, getState, dispatch }) => {
    //get user token from store
    ...
    try {
        ...
        const { data } = await axios.put(`${BaseURL}/teams/${payload?._id}`, payload, config);
...
export const deleteteamAction = createAsyncThunk('team/delete', async (payload, { rejectWithValue, getState, dispatch }) => {
    //get user token from store
    ...
    const config = {
     ...
     try {
         ...
         const { data } = await axios.delete(`${BaseURL}/teams/${payload?._id}`, config);
