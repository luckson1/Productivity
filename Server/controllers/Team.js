...
const createTeamCtrl= expressAsyncHandler(async (req, res) => {
    ...
    try {
        const team= await Team.create({name, description, teamCreator, teamId, teamMembers})
...
const updateTeamctrl = expressAsyncHandler(async (req, res) => {
    ...
    const updateDocument= {
        ...
        try {
        const team = await Team.findByIdAndUpdate(id, {name,  description, updateDocument}, { new: true })
...
module.exports ={fetchUserTeams, createTeamCtrl, fetchOneTeamCtrl, fetchAllTeams, updateTeamctrl, deleteTeamctrl}