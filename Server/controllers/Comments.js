...
const createCommentCtrl= async (req, res)=> {
    ...
    const creator= id
    try {
        const comment= await Comment.create({taskId, bugId, details, creator, commentId})
        res.json({comment})
    } catch (error) {
        res.json({error})
    }
...
const updateCommentCtrl= async (req, res)=> {
    ...
    try {
        const comment= await Comment.findOneAndUpdate({commentId: commentId}, {details}, { new: true } )
        res.json({comment})
    } catch (error) {
        res.json ({error})
    }
...
module.exports= {createCommentCtrl, updateCommentCtrl, fetchCommentsCtrl, deleteCommentCtrl}